...
def add_marker_at(the_map, segment, index, tooltip, **kwargs):
    ...
    if index == 0:
        ...
        elevation = segment['elevation-up']
        txt = f'<b>End:</b> {time}</br><b>Distance:</b> {dist:.2f} km <b>Elevation:</b> {elevation:.2f} km'
    else:
    ...
    # )
    circle = folium.Circle(location=segment['latlon'][index],
                        popup=folium.Popup(txt, max_width=250), 
                        color='black',
                        fill=True,
                        fill_color='black',
...
def add_segment_to_map(the_map, segment, color_by=None, cmap='viridis',
    ...
    if color_by is None:
        ...
        line = add_colored_line(the_map, segment, color_by, cmap=cmap,
                         line_options=line_options, min_value=min_value,
                         max_value=max_value, show_cmap=show_cmap)
    if 1 : #add infos
        time = segment['time'][0].strftime('%A %B %d, %Y: %H:%M:%S')
        dist = segment['distance'][-1] / 1000.
        elevation = segment['elevation-up']
        txt = ''
        txt = f'<b>Track :</b> {segment["name"]}</br> \
                <b>Date :</b> {time}</br> \
...
def add_marker_in_json(route_map, markers_blog_path):
    # this is the layer that eventually gets added to Map
    layer = folium.FeatureGroup(show=True, control=False)
    # load GEOJSON, but don't add it to anything
    gj = folium.GeoJson(markers_blog_path)
    ...
    for feature in gj.data['features']:
        if feature['geometry']['type'] == 'Point' : # feature['geometry']['type'] == 'Point':
            tool_tip_text = '<pre>\n'
            tool_tip_text =  str(feature['properties']["name"]) 
            tool_tip_text = tool_tip_text + '\n</pre>'
            tool_tip = folium.Tooltip(tool_tip_text)
    
            icon = folium.features.CustomIcon(f"https://www.diveandbike.fr/icons/{feature['properties']['_umap_options']['icon']}.png",
                                            icon_size=(30,40),
                                            icon_anchor=(5, 35)
                                            ) #, color="blue")
            ...
            print(feature['properties']["_umap_options"])
